...
thread_yield (void) 
{
  ...
  if (cur != idle_thread) {
    /*
	 * Keep the ready list as being ordered to ensure priority scheduling.
	 *
	 * Modified by Bogdan
     */
...
/*
 * The new priority is not the result of a donation
 */
void
thread_set_priority (int new_priority) {
  thread_set_priority_helper (thread_current (), new_priority, false);
}

/*
 * Utility function for setting a priority.
 *
 */
void thread_set_priority_helper (struct thread *cur, int new_priority, bool is_donated)
{
	enum intr_level old_level;
	old_level = intr_disable();
	...
	/*
	 * Check if we're dealing with a donation or a simple priority change.
	 */
	if (!is_donated) {

		/*
		 * If the current priority is donated, delay the priority change as it may cause deadlocks.
		 */
		if (cur->current_priority_donated && new_priority <= cur->priority) {
			/*
			 * This will be set as the donated priority is lifted.
			 */
			cur->original_priority = new_priority;
		} else {
			/*
			 * Simply increase the current priority and original priority, should not cause deadlocks.
			 */
		...
		cur->current_priority_donated = true;
	}


	/*
	 * If the current thread is in the ready queue, change its position.
	 */
	if (cur->status == THREAD_READY) {
		...
		/*
		 * Current thread is running but there might someone with a greater priority available.
		 */
		int running_candidate_priority  = list_entry (list_begin (&ready_list), struct thread, elem)->priority;

		if (cur->status == THREAD_RUNNING && running_candidate_priority > cur->priority ) {
			/*
			 * Yield the CPU to the candidate.
			 */
...
/*
 * Retrieve a thread from the all list by its id.
 *
 * Added by Bogdan.
 */
