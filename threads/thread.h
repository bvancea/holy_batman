...
/* A fake priority, used in priority donation */
...
struct thread
  {
    ...
    /* Is true if thread's priority is donated */
    bool current_priority_donated;
    struct list locks;                    /* All locks a thread holds */
    struct lock *blocked_by;         /* Thread blocked by lock */
